...
def confirm(Cart,total,Quantity,chooseBakarset,setsBakarmenu,choosePenyetset,setsPenyetmenu,chosenadd_on,add_onopt):
...
#        purchased[len(purchased)] = {"Sets": ChosenFooditem, "Add-ons": choosenaddon, "Quantity": New_Quantity,"Cost": total}
        ...
        elif choosenaddon in add_onsopt.keys():
            total = float(add_onsopt[choosenaddon][2]) * New_Quantity
            New_Cart = total
            Cart = New_Cart
            print("Item Quantity Changed to ", New_Quantity)
            print("Cart updated")
    ...
    return Cart,New_Quantity,total,choosenaddon,ChosenFooditem,chooseBakarset,setsBakarmenu,choosePenyetset,setsPenyetmenu,chosenadd_on,add_onopt
...
def displaycart():
    print(
        ...
        "|                                          CART                                               | \n"
        "+---------------------------------------------------------------------------------------------+")
    print(f"| {"SETS":<50} {"QUANTITY":<10}   {"COST":<29}|")
    print("+---------------------------------------------------------------------------------------------+")
    for i in purchased_sets:
        print(f"| {i["SET"]:<50}  {i["QUANTITY"]:<10}  ${i["COST"]:<26.2f} |")
    print("+---------------------------------------------------------------------------------------------+")
    print(f"| {"ADD-ONS":<50} {"QUANTITY":<10}   {"COST":<29}|")
    print("+---------------------------------------------------------------------------------------------+")
    for r in purchased_addons:
        print(f"| {r["ADD-ON"]:<50}  {r["QUANTITY"]:<10}  ${r["COST"]:<26.2f}|")
    print("+---------------------------------------------------------------------------------------------+")
    print(f"| {"YOUR TOTAL:":<61} ${Cart:<32.2f}|")
    print("+---------------------------------------------------------------------------------------------+")
        ...
        if confirmcartqns == "n":
            ...
            if whichtoedit == "1":
                ...
                while edit not in ["1", "2", "3"]:
                    print("Invalid input. Please try again.")
                    edit = input("CHOOSE OPTION [1/2]: ")

                if edit == "1":
                    print("SETS ORDERED:")
                    for n in range(1, len(purchased_sets) + 1):
                        print(f"|{n}) {purchased_sets[n - 1]["SET"]}")
                    #        n += 1
                ...
                if edit == "2":
                    ...
                    for n in range(1, len(purchased_addons) + 1):
                        print(f"|{n}) {purchased_addons[n - 1]["ADD-ON"]}")
                        #    n += 1
                ...
                if setsoradd_onquantity == "1":
                    print("SETS ORDERED:")
                    for n in range(1, len(purchased_sets) + 1):
                        print(f"|{n}) {purchased_sets[n - 1]["SET"]} {purchased_sets[n - 1]["QUANTITY"]}")
                        # n += 1
                    changesetquantity = int(
                        input("Enter the which set's quantity to change[enter the index number shown above only]:  "))
                    ...
                    oldcost = purchased_sets[changesetquantity - 1]["COST"] / purchased_sets[changesetquantity - 1][
                        "QUANTITY"]
                ...
                if setsoradd_onquantity == "2":
                    print("ADD-ONS ORDERED:")
                    for n in range(1, len(purchased_addons) + 1):
                        print(f"|{n}) {purchased_addons[n - 1]["ADD-ON"]} {purchased_addons[n - 1]["QUANTITY"]}")
                        #    n += 1
                    changeadd_onquantity = int(
                        input("Enter the which set's quantity to change[enter the index number shown above only]:  "))
                    newsaddonquantity = int(input(f"Enter the new quantity: "))
                    Cart -= purchased_addons[changeadd_onquantity - 1]["COST"]
                    oldcost = purchased_addons[changeadd_onquantity - 1]["COST"] / \
                              purchased_sets[changeadd_onquantity - 1]["QUANTITY"]
    ...
    if lang == "1":
        ...
        if mainmenu == "1":
            ...
            if chosenset == "1":
                ...
                if chooseBakarset in ["1","2","3","4"]:
                  ...
                  print(f"|  {ChosenFooditem:<10} {Quantity:^10} {total:>10.2f}   |")
                  Cart, New_Quantity, total, choosenaddon, ChosenFooditem, chooseBakarset, setsBakarmenu, choosePenyetset, setsPenyetmenu, chosenadd_on, add_onopt = confirm(Cart,total,Quantity, chooseBakarset, setsBakarmenu, choosePenyetset, setsPenyetmenu, chosenadd_on, add_onopt)
        ...
        if mainmenu == "2":
            ...
            if chooseadd_on in ["1","2","3","4","5","6"]:
                ...
                print(f"|  {choosenaddon:<10} {Quantity:>32} {total:>14.2f}   |")
                Cart, New_Quantity, total, choosenaddon, ChosenFooditem, chooseBakarset, setsBakarmenu, choosePenyetset, setsPenyetmenu, chosenadd_on, add_onopt = confirm(Cart, total,Quantity, chooseBakarset, setsBakarmenu, choosePenyetset, setsPenyetmenu, chosenadd_on, add_onopt)
                choicemoveon = move_on()
                if choicemoveon == "A":
                    # set_mainmenu()
        ...
        if mainmenu == "4":
            displaycart()
            #To add edit cart function here?? - to check on this and reconsider this option.....
            actionaftercart()
